...
{
    ros::init(argc, argv, "offboard_takeoff_setpoints_node");
    ros::NodeHandle nh;
    ...
    while(ros::ok())
    {
        ...
        while (OffboardClient_object.current_state.armed && OffboardClient_object.current_state.mode == "OFFBOARD")
        {
            ...
            {
                ROS_INFO("Going to 1st waypoint");
                OffboardClient_object.set_local_pos_pub.publish(OffboardClient_object.set_position_msg);
                ros::spinOnce();
                loop_rate.sleep();
            }
            ...
            while(ros::Time::now() - setpoint_timer < ros::Duration(HOVER_DURATION))
            {
                ROS_INFO("Going to 2nd waypoint");
                OffboardClient_object.set_local_pos_pub.publish(OffboardClient_object.set_position_msg);
                ros::spinOnce();
                loop_rate.sleep();
            }
            ...
            while(ros::Time::now() - setpoint_timer < ros::Duration(HOVER_DURATION))
            {
                ROS_INFO("Going to 3rd waypoint");
                OffboardClient_object.set_local_pos_pub.publish(OffboardClient_object.set_position_msg);
                ros::spinOnce();
                loop_rate.sleep();
            }
            ...
            while(ros::Time::now() - setpoint_timer < ros::Duration(HOVER_DURATION))
            {
                ROS_INFO("Going to 4th waypoint");
                OffboardClient_object.set_local_pos_pub.publish(OffboardClient_object.set_position_msg);
                ros::spinOnce();
                loop_rate.sleep();
            }
