...
{
    ros::init(argc, argv, "offboard_takeoff_land_node");
    ros::NodeHandle nh;
    ...
    while(ros::ok())
    {
        ...
        while (OffboardClient_object.current_state.armed && OffboardClient_object.current_state.mode == "OFFBOARD")
        {
            ...
            setpoint_timer = ros::Time::now();
            OffboardClient_object.set_position_msg.pose.position.z = 1.0;
            ...
            setpoint_timer = ros::Time::now();
            OffboardClient_object.set_position_msg.pose.position.z = 0.5;

            while(ros::Time::now() - setpoint_timer < ros::Duration(HOVER_DURATION))
            {
                ROS_INFO("Hovering at 0.5 [m]");
                OffboardClient_object.set_local_pos_pub.publish(OffboardClient_object.set_position_msg);
                ros::spinOnce();
                loop_rate.sleep();
            }
            ...
            setpoint_timer = ros::Time::now();
            OffboardClient_object.set_position_msg.pose.position.z = 0.2;

            while(ros::Time::now() - setpoint_timer < ros::Duration(HOVER_DURATION))
            {
                ROS_INFO("Hovering at 0.2 [m]");
                OffboardClient_object.set_local_pos_pub.publish(OffboardClient_object.set_position_msg);
                ros::spinOnce();
                loop_rate.sleep();
            }
